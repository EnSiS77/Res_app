                    ...
                    <form method="POST" action="{{ route('admin.reservation.update', $reservation->id) }}">
                        ...
                        <div class="sm:col-span-6 mt-2">
                            <label for="res_date" class="block text-sm font-medium text-gray-700">Дата
                                резервации</label>
                            <div class="mt-1">
                                <input type="datetime-local" id="res_date" name="res_date" value="{{ \Carbon\Carbon::parse($reservation->res_date)->format('Y-m-d\TH:i:s') }}"
                        ...
                        <div class="sm:col-span-6 mt-2">
                            <label for="guest_number" class="block text-sm font-medium text-gray-700">Количество
                                гостей</label>
                            <div class="mt-1">
                                ...
                                <select id="table_id" name="table_id"
                                    ...
                                    @foreach ($tables as $table)
                                        <option value="{{ $table->id }}" @selected($table->id ==  $reservation->table_id)>{{ $table->name }}
                                            ({{ $table->guest_number }} места)</option>
                                    @endforeach
